...
import { isRoot, isReply } from "ssb-thread-schema";
import lodash from "lodash";
import prettyMs from "pretty-ms";
import pullAbortable from "pull-abortable";
import pullParallelMap from "pull-paramap";
import pull, { PossibleSource } from "pull-stream";
...
export default ({ cooler, isPublic }) => {
  ...
  const getAbout = async ({ key, feedId }): Promise<string | null> => {
    ...
    return new Promise<string | null>((resolve, reject) =>
      pull(
        source,
        pull.find(
          (message: message) => message.value.content[key] !== undefined,
          (err, message: message) => {
            if (err) {
              reject(err);
            } else {
      ...
      getAbout({ feedId: feed, key: "image" })
        .then((img) => {
          ...
          if (
            ...
            typeof img !== "string" &&
            typeof img === "object" &&
            //@ts-ignore //TODO: typing may be bad here, or the default [.link] part may be wrong
          ...
          models.friend
            ...
            .then((rel) => {
              ...
              feeds_named.push({ feed, name, rel, exImg });
  ...
  const blob = {
    ...
    getResolved: async ({ blobId }) => {
      const bufferSource: PossibleSource<Buffer> = await models.blob.get({ blobId });
  ...
  const meta = {
    ...
    peers: async () => {
      ...
      return new Promise<PeerMetadata[]>((resolve, reject) => {
        pull(
          peersSource,
          ...
          pull.collect((err, val) => {
            if (err) return reject(err);
            //@ts-ignore //HACK: I'll just have to ignore this for now, sadly
            resolve(val[0]);
          })
        );
      });
  ...
  const socialFilter = async ({
    ...
    const relationshipObject = await new Promise<About>((resolve, reject) => {
    ...
    return pull.filter((message: message) => {
  ...
  const transform = (ssb, messages, myFeedId) =>
    Promise.all(
      messages.map(async (msg) => {
        ...
        const rawVotes = await new Promise<IVoteRef[]>((resolve, reject) => {
          pull(
            referenceStream,
            pull.filter(
              (ref: IVoteRef) =>
  ...
  const post = {
    ...
    topicComments: async (rootId, customOptions = {}): Promise<message[]> => {
      const ssb = await cooler.open();
    ...
    latestThreads: async () => {
      ...
      const messages = await new Promise((resolve, reject) => {
        pull(
          ...
          pull.filter((message: message) => message.value.meta.thread!=undefined&&message.value.meta.thread.length > 1),
          pull.collect((err, collectedMessages) => {
            if (err) {
              reject(err);
            } else {
    ...
    popular: async ({ period }) => {
      ...
      const messages = await new Promise((resolve, reject) => {
        pull(
          ...
          pull.reduce(
            (acc, cur) => {
              const author:string = cur.value.author;
              const target:string = cur.value.content.vote.link;
              const value:number = cur.value.content.vote.value;
            ...
            (err, obj) => {
              ...
              const adjustedObj = Object.entries(obj).reduce(
                (acc, [author, values]) => {
                  ...
                  //@ts-ignore //TODO: figure out how to properly type this nightmare
    ...
    fromThread: async (msgId: string, customOptions = {}) => {
      ...
      return ssb
        ...
        .then(async (rawMsg) => {
          ...
          const getRootAncestor = (msg) =>//TODO: squash this recursion
            new Promise<message|message[]>((resolve, reject) => {
              if (msg.key == null) {
          ...
          const getDirectDescendants = (key) =>
            new Promise<message[]>((resolve, reject) => {
              ...
              pull(
                ...
                pull.collect((err, messages: message[]) => {
                  if (err) {
                    reject(err);
                  } else {
          ...
          const deepDescendants = await getDeepDescendants(Array.isArray(rootAncestor)?rootAncestor[0].key:rootAncestor.key);//HACK:added [0], this may break things

          const allMessages = Array.isArray(rootAncestor)?[...rootAncestor, ...deepDescendants]:[ rootAncestor, ...deepDescendants].map(
            (message: message) => {
    ...
    get: async (msgId: string, customOptions = {}) => {
      debug("get: %s", msgId);
      const ssb = await cooler.open();
	...
	pull(
      ...
      const channels = messages.map((msg:message) => {
        return {
      ...
      let subbedChannels:string[] = [];

      channels.forEach((ch) => {
        if (ch.subscribed && ch.channel && !subbedChannels.includes(ch.channel)) {
          subbedChannels.push(ch.channel);
        }
        if (ch.subscribed === false && ch.channel && subbedChannels.includes(ch.channel)) {
